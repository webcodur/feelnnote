      ...
      <div className="hidden lg:block">
        <ClassicalBox className="p-6 flex flex-col items-center">
          ...
          <nav className="w-full relative" onMouseLeave={() => setHoveredIndex(null)}>
            {/* 슬라이딩 인디케이터 (호버 따라다님) */}
            {(hoveredIndex ?? activeIndex) >= 0 && (
              <div className="absolute left-0 right-0 pointer-events-none transition-all duration-300 ease-out" style={{ top: (hoveredIndex ?? activeIndex) * (ITEM_HEIGHT + ITEM_GAP), height: ITEM_HEIGHT }}>
            ...
            <div className="relative z-10 flex flex-col gap-2">
              {tabs.map((tab, index) => {
                ...
                const isIndicatorHere = hoveredIndex === index || (hoveredIndex === null && isTabActive);
                return (
                  <Link
                    key={tab.href}
                    href={tab.href}
                    ...
                    className={cn(
                      ...
                      isTabActive ? "text-accent font-black" : isIndicatorHere ? "text-accent" : "text-text-secondary hover:text-text-primary",
                    )}
                  ...
                  >
                    <tab.icon
                      ...
                      className={cn(
                        ...
                        isTabActive ? "text-accent drop-shadow-[0_0_10px_rgba(212,175,55,0.7)]" : isIndicatorHere ? "text-accent opacity-80" : "opacity-40 group-hover:opacity-80",
                      )}
                    />
                    <span className={cn(isTabActive && "text-glow")}>{tab.label}</span>
                  </Link>
                );
              })}
            </div>
          ...
          {tabs.map((tab) => {
            ...
            return (
              <Link
                ...
                href={tab.href}
                className={cn(
                  "relative flex items-center justify-center px-2 py-3.5 whitespace-nowrap",
                  isTabActive
