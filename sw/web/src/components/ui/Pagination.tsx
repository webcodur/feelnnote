...
const STYLES = {
  ...
  // Segmented Button
  segmentBtn: "h-8 w-8 flex items-center justify-center text-text-secondary/70 hover:text-accent hover:bg-white/10 disabled:opacity-20 disabled:cursor-not-allowed first:rounded-l-md last:rounded-r-md active:scale-90",
  ...
  inputWrapper: "h-8 flex items-center bg-white/5 border border-white/5 focus-within:border-accent/40 rounded-lg overflow-hidden w-[72px]",
  input: "h-full w-full bg-transparent text-center text-sm font-bold text-accent focus:outline-none placeholder:text-text-secondary/30 px-1 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none",
  submitBtn: "h-full w-8 flex items-center justify-center text-text-secondary/40 hover:text-accent shrink-0",
} as const;
// #endregion
